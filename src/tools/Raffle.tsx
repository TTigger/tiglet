...
const L = {
  zh: {
    importFile: '匯入 Excel / CSV 名單',
    privacy: '檔案只在你的瀏覽器處理，不會上傳。',
    errTooFew: '檔案資料不足（需要標題列 + 至少一筆資料）',
    errRead: '無法讀取檔案，請確認格式為 .xlsx / .xls / .csv',
    pickColumn: '選擇名單所在的欄位：',
    placeholder: '每行一個名字，或從上方匯入 Excel：\n王小明\n陳大文\n林美麗',
    prizeName: '獎品名稱',
    prizePlaceholder: '例如：頭獎 / iPhone',
    drawCountBefore: '本輪抽出',
    drawCountAfter: '位',
    excludeWon: '排除已中獎者',
    marqueeIdle: '🎁 準備抽獎',
    drawing: '抽獎中…',
    drawBtn: (n: number) => `開始抽獎（剩餘 ${n} 人）`,
    winners: '🎉 中獎名單',
    clear: '清除',
    round: (n: number) => `第 ${n} 輪`,
  },
  en: {
    importFile: 'Import an Excel / CSV list',
    privacy: 'Files are processed in your browser only — never uploaded.',
    errTooFew: 'Not enough data in the file (needs a header row + at least one data row)',
    errRead: 'Could not read the file. Make sure it is .xlsx / .xls / .csv',
    pickColumn: 'Pick the column that contains the names:',
    placeholder: 'One name per line, or import from Excel above:\nAlice\nBob\nCarol',
    prizeName: 'Prize name',
    prizePlaceholder: 'e.g. Grand prize / iPhone',
    drawCountBefore: 'Draw',
    drawCountAfter: 'winner(s) this round',
    excludeWon: 'Exclude previous winners',
    marqueeIdle: '🎁 Ready to draw',
    drawing: 'Drawing…',
    drawBtn: (n: number) => `Start the draw (${n} left)`,
    winners: '🎉 Winners',
    clear: 'Clear',
...
export default function Raffle({ locale = 'zh' }: { locale?: Locale }) {
  const t = L[locale];
  ...
  async function onFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (parsed.length < 2) { setError(t.errTooFew); return; }
      setRows(parsed);
  ...
  function draw() {
    ...
    const label = prize.trim() || t.round(draws.length + 1);
    ...
    <div className="mx-auto max-w-lg">
      ...
      {headers && (
        <div className="mb-4 rounded-[var(--radius-card)] border border-edge bg-surface p-4">
          <p className="mb-2 text-sm text-ink">{t.pickColumn}</p>
      ...
      <div className="mt-4 grid gap-3 rounded-[var(--radius-card)] border border-edge bg-surface p-4 sm:grid-cols-2">
        <label className="text-sm text-ink sm:col-span-2">{t.prizeName}
          <input value={prize} onChange={(e) => setPrize(e.target.value)} placeholder={t.prizePlaceholder} className="mt-1 w-full rounded border border-edge bg-bg px-3 py-1.5 text-ink outline-none focus:border-accent" />
        </label>
        <label className="text-sm text-ink">{t.drawCountBefore}
        ...
        {rolling ? (
          ...
          <div className="flex h-full items-center overflow-hidden">
            <div className="marquee-track" style={{ animationPlayState: names.length ? 'running' : 'paused' }}>
              {[...names, ...names, t.marqueeIdle].map((nm, i) => (
      ...
      <button onClick={draw} disabled={pool.length === 0 || rolling} className="mt-4 w-full rounded-lg bg-accent py-3 text-white transition-colors hover:bg-[var(--color-accent-hover)] disabled:opacity-50">
        {rolling ? t.drawing : t.drawBtn(pool.length)}
      </button>

      {draws.length > 0 && (
        <div className="mt-6 rounded-[var(--radius-card)] border border-edge bg-surface p-6">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="font-serif text-xl text-ink">{t.winners}</h2>
            <button onClick={reset} className="text-sm text-muted hover:text-accent">{t.clear}</button>
          </div>
