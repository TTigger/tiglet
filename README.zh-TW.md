...
你的輸入不會離開你的裝置。（例外：匯率換算會抓取公開匯率，以及匿名的
頁面瀏覽統計；詳見[隱私](#隱私)。）
...
| 單車 | **賽段剖面圖** | 上傳 GPX／FIT／TCX（或直接拖曳）生成環法風格賽段剖面圖——自動偵測爬坡並分級（HC～四級）、坡度分布與最陡路段，一鍵下載 PNG；完全在瀏覽器本機解析 |
...
目前只有一支 `fetch-meta`，供網址健檢抓取目標頁的 HTML（瀏覽器因 CORS 無法直接抓）。
這裡的函式必須維持無狀態——不接資料庫、不記錄使用者輸入。

## 隱私

Tiglet 幾乎完全在前端運作——無登入、無 cookie、無廣告。抽獎的 Excel 匯入、
QR 產生器、圖片壓縮／縮放、圖片取色，以及其他所有工具，都在你的瀏覽器本機
處理資料，不會上傳任何東西。
